...
def extract_command(message: dict[str, Any]) -> str:
    text = str(message.get("text") or "").strip()
    if not text:
        return ""
    entities = message.get("entities") or []
    for entity in entities:
        if entity.get("type") == "bot_command" and int(entity.get("offset", -1)) == 0:
            length = int(entity.get("length", 0))
            cmd = text[:length].strip().lower()
            return cmd
...
def command_matches(command: str, *aliases: str) -> bool:
    if not command:
        return False
...
"
    ...
    if command_matches(command, "/pause"):
        with state.lock:
    ...
    if command_matches(command, "/resume"):
        with state.lock:
    ...
    if command_matches(command, "/reload"):
        load_state()
        send_message(chat_id, "Конфиг перечитан. Текущий код работает.")
        return

    if command_matches(command, "/test"):
...
def process_group_fun_commands(message: dict[str, Any]) -> bool:
    command = extract_command(message)
    if command_matches(command, "/anekdot", "/анекдот", "/joke"):
        send_message(
    ...
    if normalized in {"анекдот", "анекдот!", "шутка", "шутка!", "joke"}:
        send_message(
            int((message.get("chat") or {}).get("id", 0)),
            choose(JOKES),
            thread_id=message.get("message_thread_id"),
